...
def test_auto_increment_raw(default_config):
    with MimeoContextManager(default_config) as mimeo_manager:
        ...
        identifier = MimeoUtilRenderer.render_raw("auto_increment")
...
def test_auto_increment_parametrized_default(default_config):
    with MimeoContextManager(default_config) as mimeo_manager:
        ...
        identifier = MimeoUtilRenderer.render_parametrized({"_name": "auto_increment"})
        assert identifier == "00001"
        identifier = MimeoUtilRenderer.render_parametrized({"_name": "auto_increment"})
        assert identifier == "00002"
...
def test_auto_increment_parametrized_with_pattern(default_config):
    with MimeoContextManager(default_config) as mimeo_manager:
        ...
        identifier = MimeoUtilRenderer.render_parametrized({"_name": "auto_increment", "pattern": "{}"})
        assert identifier == "1"
        identifier = MimeoUtilRenderer.render_parametrized({"_name": "auto_increment", "pattern": "MYID/{}"})
        assert identifier == "MYID/2"
        identifier = MimeoUtilRenderer.render_parametrized({"_name": "auto_increment", "pattern": "MYID_{:010d}"})
...
def test_auto_increment_parametrized_with_non_str_pattern(default_config):
    with MimeoContextManager(default_config) as mimeo_manager:
        ...
        with pytest.raises(AttributeError) as err:
            MimeoUtilRenderer.render_parametrized({"_name": "auto_increment", "pattern": 1})
