...
def test_first_name_parametrized_with_allow_duplicates(default_config):
    mimeo_db = MimeoDB()
    first_names = [first_name.name for first_name in iter(mimeo_db.get_first_names())]
    with MimeoContextManager(default_config) as mimeo_manager:
        context = mimeo_manager.get_context("SomeEntity")
        ...
        first_name = UtilsRenderer.render_parametrized({"_name": "first_name", "allow_duplicates": True})
        assert first_name in first_names


def test_first_name_parametrized_with_allow_duplicates_and_invalid_sex(default_config):
    with MimeoContextManager(default_config) as mimeo_manager:
        context = mimeo_manager.get_context("SomeEntity")
        mimeo_manager.set_current_context(context)

        with pytest.raises(InvalidSex) as err:
            UtilsRenderer.render_parametrized({"_name": "first_name", "allow_duplicates": True, "sex": "N"})

        assert err.value.args[0] == "Invalid sex (use M, F, Male or Female)!"


def test_first_name_parametrized_with_allow_duplicates_and_sex_m(default_config):
    mimeo_db = MimeoDB()
    ...
    with MimeoContextManager(default_config) as mimeo_manager:
        ...
        first_name = UtilsRenderer.render_parametrized({"_name": "first_name", "allow_duplicates": True, "sex": "M"})
        assert first_name in gbr_first_names


def test_first_name_parametrized_with_allow_duplicates_and_sex_f(default_config):
    mimeo_db = MimeoDB()
    ...
    with MimeoContextManager(default_config) as mimeo_manager:
        ...
        first_name = UtilsRenderer.render_parametrized({"_name": "first_name", "allow_duplicates": True, "sex": "F"})
        assert first_name in gbr_first_names


def test_first_name_parametrized_with_allow_duplicates_and_sex_male(default_config):
    mimeo_db = MimeoDB()
    gbr_first_names = [first_name.name for first_name in iter(mimeo_db.get_first_names_by_sex('M'))]
    with MimeoContextManager(default_config) as mimeo_manager:
        context = mimeo_manager.get_context("SomeEntity")
        ...
        first_name = UtilsRenderer.render_parametrized({"_name": "first_name", "allow_duplicates": True, "sex": "Male"})
        assert first_name in gbr_first_names


def test_first_name_parametrized_with_allow_duplicates_and_sex_female(default_config):
    mimeo_db = MimeoDB()
    gbr_first_names = [first_name.name for first_name in iter(mimeo_db.get_first_names_by_sex('F'))]
    with MimeoContextManager(default_config) as mimeo_manager:
        context = mimeo_manager.get_context("SomeEntity")
        ...
        first_name = UtilsRenderer.render_parametrized({"_name": "first_name", "allow_duplicates": True, "sex": "Female"})
        assert first_name in gbr_first_names
